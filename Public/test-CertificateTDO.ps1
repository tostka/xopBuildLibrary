#region TEST_CERTIFICATETDO ; #*------v FUNCTION test-CertificateTDO v------
function test-CertificateTDO {
            <#
            .SYNOPSIS
            test-CertificateTDO -  Tests specified certificate for certificate chain and revocation (either as a file, or in cert store)
            .NOTES
            Version     : 0.63
            ...
            REVISIONS
            * 2:38 PM 7/24/2025 add: support for non-cert object passed in with a thumprint specification; 
                 add: CBH demo of test notbefore/notafter; add alias test-CertificateFileTDO
            ...
            * 9:34 AM 8/22/2024 Vadims Podans posted poshcode.org copy from web.archive.org, grabbed 11/2016 (orig dates from 2009, undated beyond copyright line)
            ...
            test-CertificateTDO -  Tests specified certificate for certificate chain and revocation (either as a file, or in cert store)
                ...
                - in daterange, (fails echo: 'A required certificate is not within its validity period when verifying against the current system clock or the timestamp in the signed file.')
            ...
            checking is not performed.    
            .EXAMPLE
            PS> $cresult = test-CertificateTDO -Certificate 'D:\cab\mymail-torolab-com-(10-10-2025).pfx' ;           
            PS> if((get-date ) -lt $cresult.NotAfter -AND (get-date ) -gt $cresult.NotBefore){
            PS>     write-host -foregroundcolor green "Certificate is within use window (notbefore:$($cresult.NotBefore)/notafter:$($cresult.NotAfter))" ; 
            PS> }else{
            PS>     write-warning "Certificate is OUTSIDE OF use window (notbefore:$($cresult.NotBefore)/notafter:$($cresult.NotAfter))" ; 
            PS> } ; 
            Demo testing in-window date status for certificate
            .LINK
            ...
            PROCESS {
                foreach($item in $Certificate){
                    if ($item -is [System.Security.Cryptography.X509Certificates.X509Certificate2]) {
                        ...
                        return $report ;
                    }elseif($item.thumbprint){
                        $smsg = "-Thumprint specified: Attempting to resolve against LocalMachine, and then CurrentUser..." ;
                        if(gcm Write-MyOutput -ea 0){Write-MyOutput $smsg } else {
                            if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info }
                            else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                            #Levels:Error|Warn|Info|H1|H2|H3|H4|H5|Debug|Verbose|Prompt|Success
                        } ;
                        if($lmmatch = gci Cert:\LocalMachine -recur | ?{$_.thumbprint -eq $item.thumbprint}){
                            $smsg = "-Thumprint matched to:`n$(($lmmatch |out-string).trim())" ; 
                            if(gcm Write-MyOutput -ea 0){Write-MyOutput $smsg } else {
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info }
                                else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                #Levels:Error|Warn|Info|H1|H2|H3|H4|H5|Debug|Verbose|Prompt|Success
                            ...
                            $report = _getstatus_ $status $chain $item   ; 
                            return $report ;
                        }elseif($cumatch = gci Cert:\CurrentUser -recur | ?{$_.thumbprint -eq $item.thumbprint}){
                            $smsg = "-Thumprint matched to:`n$(($cumatch |out-string).trim())" ; 
                            if(gcm Write-MyOutput -ea 0){Write-MyOutput $smsg } else {
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level Info }
                                else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                #Levels:Error|Warn|Info|H1|H2|H3|H4|H5|Debug|Verbose|Prompt|Success
                            ...
                            $smsg = "Unable to locate specified Thumbprint: $($item.thumbprint) in eather LM or CU stores!" ;
                            if(gcm Write-MyWarning -ea 0){Write-MyWarning $smsg } else {
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN -Indent}
                                else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                            } ;
...
#endregion TEST_CERTIFICATETDO ; #*------^ END FUNCTION test-CertificateTDO  ^------

# SIG # Begin signature block
# MIIELgYJKoZIhvcNAQcCoIIEHzCCBBsCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU1A+OYoK36PYZDSBifDUb7hrI
# bZKgggI4MIICNDCCAaGgAwIBAgIQWsnStFUuSIVNR8uhNSlE6TAJBgUrDgMCHQUA
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQ9L1KS
# NxpI72B+sqAVvehsJaig9zANBgkqhkiG9w0BAQEFAASBgGDfd85Dcv0tfKl7KAEv
# 2wZ06/dF3bauqagvgPRyKtjLtJJ8Svk+60X29IvFWaKZIr+5f8yCJPsO5ry1MEhX
# xwPMm6JkGkKS0XoV5bxFYIWgwat/RYUtavp6S0u0olIPr8Di7aa+xIq9ZXbneO2u
# DKF2W7/i5otaULviXizZdl82
