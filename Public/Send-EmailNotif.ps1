#region SEND_EMAILNOTIF ; #*------v FUNCTION Send-EmailNotif v------
Function Send-EmailNotif {
            ...
            AddedTwitter:	URL
            REVISIONS
            *1:55 PM 8/18/2025 exempt smtpserver discovery, if the -smtpserver param is populated in call (bypass logic, use hard-coded solution).
             added ServerUnreachable error trap ; swapped wly calls for wlt; 
             expanded logic, to pull digits-ordered Age-ordered array of hubtransport/edgetransport servers, and configured unavailable catch block to retry through the list. (untested, built in lab edge - no redundancy). 
            ...
            * send-emailnotif.ps1: * 1:49 PM 11/23/2020 wrapped the email hash dump into a write-host cmd to get it streamed into the log at the point it's fired. 
            ...
            if(-not $SMTPServer){
                if ( ($myBox -contains $env:COMPUTERNAME) -OR ($AdminJumpBoxes -contains $env:COMPUTERNAME) ) {
                    ...
                    # Age-ordered hub server array more predictable, use the lowest numeric server number - older servers more likely to be non-maint mode, full functional
                    if($env:userdomain -eq $env:computername){
                        #$ExHubs = get-exchangeserver | ?{$_.IsEdgeServer} | select 'name',@{Name='digits';Expression={if($_ -match '[\d]+'){[int]$matches[0] } }} | sort digits | select -expand name ;
                        # always use self
                        $ExHubs = get-exchangeserver | ?{$_.IsEdgeServer} | select -expand fqdn ; 
                    }else{
                        $ExHubs = get-exchangeserver | ?{$_.IsHubTransportServer} | select 'name',@{Name='digits';Expression={if($_ -match '[\d]+'){[int]$matches[0] } }} | 
                            sort digits | select -expand name ;
                    }
                ...
                $smsg = "-SmtpServer: $($SmtpServer) specified (no-autodiscovery)" ;
            ...
            if ($SmtpAttachment) {
                if ($SmtpAttachment -isnot [system.array]) {
                    if (test-path $SmtpAttachment) {$validatedAttachments += $SmtpAttachment }
                        ...
                        $smsg = "UNABLE TO GCI ATTACHMENT:$($SmtpAttachment)" 
                        if(gcm Write-MyWarning -ea 0){Write-MyWarning $smsg } else {
                            if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN} else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                        } ;
                    }
                    ...
                    foreach ($attachment in $SmtpAttachment) {
                        if (test-path $attachment) {$validatedAttachments += $attachment }
                            ...
                            $smsg = "UNABLE TO GCI ATTACHMENT:$($attachment)" 
                            if(gcm Write-MyWarning -ea 0){Write-MyWarning $smsg } else {
                                if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN} else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                            } ;
                        }  ;
            ...
            TRY {
                ...
                $ErrTrapd=$Error[0] ;
                $rgxSMTPServerMaintErr = "Service\snot\savailable,\sclosing\stransmission\schannel\.\sThe\sserver\sresponse\swas:\s4\.3\.2\sService\snot\sactive" ; 
                $rgxSMTPServerMaintUnreachableErr = "Unable\sto\sconnect\sto\sthe\sremote\sserver" ; 
                if($ErrTrapd.Exception -match $rgxSMTPServerMaintErr  -OR $ErrTrapd.Exception -match $rgxSMTPServerMaintUnreachableErr){
                    $smsg = "SMTPServer :$($SmtpServer) is unavailable (Maint Mode?)"
                    if($ExHubs -is [array]){
                        $ExHubs = $ExHubs |?{$_ -notmatch $env:COMPUTERNAME} ; 
                        $RetryNo = 0 ; 
                        foreach ($ExH in $ExHubs){
                            $RetryNo++ ; 
                            TRY {                                
                                $sdMM.SMTPServer = $ExH ; 
                                $smsg = "Mail Retry:$($sdMM.SMTPServer )" ;
                                if(gcm Write-MyOutput -ea 0){Write-MyOutput $smsg } else {
                                    if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level H1 } else{ write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                    #Levels:Error|Warn|Info|H1|H2|H3|H4|H5|Debug|Verbose|Prompt|Success
                                } ;
                            ...
                            if($RetryNo = $ExHubs.count){
                                $smsg = "Exhausted alt Smtp Servers: Aborting mail send attempt"
                                if(gcm Write-MyWarning -ea 0){Write-MyWarning $smsg } else {
                                    if ($logging) { Write-Log -LogContent $smsg -Path $logfile -useHost -Level WARN} else{ write-WARNING "$((get-date).ToString('HH:mm:ss')):$($smsg)" } ;
                                } ;
                            }else{
...
#endregion SEND_EMAILNOTIF ; #*------^ END FUNCTION Send-EmailNotif  ^------

# SIG # Begin signature block
# MIIELgYJKoZIhvcNAQcCoIIEHzCCBBsCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUag1fTr4ZR6q0nnkWKaKEuYXT
# vpygggI4MIICNDCCAaGgAwIBAgIQWsnStFUuSIVNR8uhNSlE6TAJBgUrDgMCHQUA
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTL4ebw
# Xgf0FX3AuLlnC1HesX9dzjANBgkqhkiG9w0BAQEFAASBgB1rH33SYnd3oSRfAJm7
# iFItNdrlWfHoR38QVeJjh/wTAsFZLptT3xpmMhaOwUJylZHUTe5qWvoNDsym1ai5
# IyprZD9dON1kZI3mmaboibXNz8BiV3mKa1pE0ghS8S2yqUazbLwrx0PtVjTlnaZ5
# CbO2DAXg4P1gN+tXh4aiF/oc
